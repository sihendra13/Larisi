/**
 * Larisi - Duitku Payment Integration (Sandbox)
 * Digunakan untuk keperluan verifikasi Payment Gateway Duitku.
 */

const DUITKU_CONFIG = {
    merchantCode: 'DS30544',
    apiKey: '01a6dcb08d58cbad3a2edd90253c89f5',
    isSandbox: true
};

// Fungsi MD5 sederhana untuk generate signature (hanya untuk keperluan sandbox/demo)
function md5(string) {
    ...
    function md5blk(s) {
        var i, a = [];
        for (i = 0; i < 64; i += 4) a[i >> 2] = s.charCodeAt(i) + (s.charCodeAt(i + 1) << 8) + (s.charCodeAt(i + 2) << 16) + (s.charCodeAt(i + 3) << 24);
        return a;
    }
...
/**
 * Memulai proses pembayaran Duitku Pop
 * @param {string} plan - Nama paket (starter/pro)
 * @param {number} amount - Harga paket dalam Rupiah
 */
