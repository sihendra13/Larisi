...
function showLaunchModal() {
  ...
  var noCatNudge  = !catNudge || !catNudge.classList.contains('visible');
...
async function _doLaunch(campNameOverride) {
  ...
  var isScheduled  = !!window._scheduledPostTime;
  var campaignData = {
    nama:        campName,
    kecamatan:   locShort,
  ...
  if (isScheduled) {
    ...
    campaignData.scheduled_at = window._scheduledPostTime;
  }
  ...
  var newCamp = {
    ...
    thumbColor:   '#791ADB',
    thumbUrl:     thumbUrl,
    launchTime:   isScheduled ? 'Dijadwalkan' : 'Baru saja',
    scheduled_at: isScheduled ? window._scheduledPostTime : null,
    aiOpening:    isScheduled
      ? 'Iklan <strong>' + campName + '</strong> telah dijadwalkan untuk ditayangkan pada ' + new Date(window._scheduledPostTime).toLocaleString('id-ID') + '.\n\nSaya akan mulai memantau performa iklan begitu iklan ini aktif ditayangkan.'
      : 'Iklan <strong>' + campName + '</strong> sudah live!\n\nAudiens tersedia: <strong>' + reachText + '</strong> di area kamu, jangkauan mulai dihitung begitu iklan ditayangkan.\n\nSaya pantau tren di <strong>' + (locShort || 'area target') + '</strong> via <strong>' + platLabel + '</strong> dan akan notify kamu kalau ada yang perlu dioptimalkan.',
    aiChips:      isScheduled ? ['Lihat detail jadwal', 'Bagikan ke tim'] : ['Lihat proyeksi', 'Optimalkan targeting', 'Bagikan ke tim'],
    aiChipResponses: isScheduled ? {
      'Lihat detail jadwal': 'Iklan dijadwalkan pada:\n\nWaktu: <strong>' + new Date(window._scheduledPostTime).toLocaleString('id-ID') + '</strong>\nPlatform: <strong>' + platLabel + '</strong>\nStatus: Terjadwal otomatis\n\nAnda dapat membatalkan atau mengubah jadwal sebelum waktu tayang.',
  ...
  window._scheduledPostTime = null; // Reset scheduling override
}
...
function showLaunchingModal(campName, onDone) {
  ...
  var isSched = !!window._scheduledPostTime;
  var loadingTitle = loading.querySelector('.ll-loading-title');
  if (loadingTitle) loadingTitle.textContent = isSched ? 'Menjadwalkan Iklan...' : 'Meluncurkan Iklan...';
  var successTitle = success.querySelector('.ll-success-title');
  if (successTitle) successTitle.textContent = isSched ? 'Iklan berhasil dijadwalkan!' : 'Iklan berhasil diluncurkan!';
...
function openScheduleModal() {
  ...
  var defaultTime = pad(tomorrow.getHours()) + ':' + 
...
function confirmSchedule() {
  ...
  if (!dateVal || !timeVal) {
    showTopToast('⏳ Pilih tanggal dan waktu terlebih dahulu', 'warning');
    return;
  }

  var schedDateTime = new Date(dateVal + 'T' + timeVal);
  if (isNaN(schedDateTime.getTime())) {
    showTopToast('⏳ Format tanggal atau waktu tidak valid', 'warning');
    return;
  }
  if (schedDateTime <= new Date()) {
    showTopToast('⏳ Waktu jadwal harus di masa depan', 'warning');
    return;
  }

  closeScheduleModal();
  
  // Set global variable for override
  window._scheduledPostTime = schedDateTime.toISOString();

  // Cek Status Berlangganan & Gembok (Paywall) - sama seperti launchRadar
  const profile = JSON.parse(localStorage.getItem('radar_user_profile') || '{}');
  const userEmail = (profile && profile.email) ? profile.email.toLowerCase().trim() : '';
  const isTester = (userEmail === 'halo@larisi.id');
  ...
  // Panggil doLaunch dengan nama default
  _doLaunch(campName);
}
