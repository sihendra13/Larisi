...
function switchMenu(view) {
  ...
  if (view === 'monitor') {
    ...
    if (typeof startScheduledStatusChecker === 'function') startScheduledStatusChecker();
...
function startScheduledStatusChecker() {
  if (_scheduledStatusCheckerTimer) clearInterval(_scheduledStatusCheckerTimer);
  _scheduledStatusCheckerTimer = setInterval(function() {
    var now = new Date();
    var changed = false;
    CAMPAIGNS.forEach(function(c) {
      if (c.status === 'scheduled' && c.scheduled_at) {
        var schedTime = new Date(c.scheduled_at);
        if (schedTime <= now) {
          c.status = 'running';
          ...
          c.launchTime = days[d.getDay()] + ', ' + d.getDate() + ' '
            + months[d.getMonth()] + ' ' + d.getFullYear()
            + ' · ' + String(d.getHours()).padStart(2,'0') + '.'
            + String(d.getMinutes()).padStart(2,'0');

          c.aiOpening = 'Iklan <strong>' + c.name + '</strong> sudah live!\n\nJangkauan mulai dihitung begitu iklan ditayangkan.';
          c.aiChips = ['Lihat performa', 'Optimalkan targeting', 'Bagikan ke tim'];
          c.aiChipResponses = {};

          if (typeof updateCampaignStatus === 'function' && c.supabase_id) {
            updateCampaignStatus(c.supabase_id, 'active');
          }
          ...
          if (activeCampaignId === c.id) {
            ...
            if (pill) {
              pill.innerHTML =
                ...
                +   '<span style="font-size:11px;font-weight:700;color:#5b21b6;">' + c.name + '</span>'
                + '</div>';
            }
          }
...
window.startScheduledStatusChecker = startScheduledStatusChecker;
...
async function _loadAnalyticsForCard(campaign) {
  ...
  try {
    ...
    var _sbStorageDom2 = RADAR_CONFIG.SUPABASE_URL + '/storage/';
    var _hasPermThumbEng2 = campaign.thumbUrl && (
      campaign.thumbUrl.startsWith('data:') ||
      campaign.thumbUrl.startsWith(_sbStorageDom2)
    );
    if (mediaUrl && !_hasPermThumbEng2) {
      campaign.thumbUrl = mediaUrl;
      localStorage.setItem('radar_thumb_' + campaign.id, mediaUrl);
